...
export class MovieScraper {
  ...
  static async getMovies(page: number = 1, limit: number = 50) {
    ...
    return cacheManager.getOrSet(cacheKey, async () => {
      try {
        ...
        console.log(`Attempting to scrape movies with Puppeteer for page ${page}`);
   ...
   */
  private static async scrapeMoviesWithPuppeteer(pageNum: number = 1): Promise<any[]> {
    let browser: Browser | null = null;
    let page: Page | null = null;

    try {
      console.log(`Launching Puppeteer browser for movies`);
      ...
      console.log(`Navigating to movies URL: ${moviesUrl}`);
      ...
      console.log(`Movies page content retrieved, length:`, content.length);
      ...
      console.log(`Found ${uniqueAnimes.length} unique movies (from ${animes.length} total)`);
