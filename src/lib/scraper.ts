...
import { Anime, AnimeDetail, Episode, Genre, StreamServer, ApiResponse } from '@/types/anime';
import axiosInstance from './axios';
import { cacheManager } from './cache';
import { SITE_CONFIG, isValidSiteUrl, buildSiteUrl } from './config';
import {
  HomeScraper,
  AnimeListScraper,
  AnimeDetailScraper,
...
// Helper function to safely get anime ID from element
const getAnimeId = ($el: any): string => {
  const dataId = $el.data('id');
  if (dataId && typeof dataId === 'string') return dataId;

  const hrefId = $el.find('a').attr('href')?.split('/').filter(Boolean).pop();
