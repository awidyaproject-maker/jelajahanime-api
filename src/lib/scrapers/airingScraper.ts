...
export class AiringScraper {
  ...
  static async getAiringAnime(page: number = 1, limit: number = 20) {
    ...
    return cacheManager.getOrSet(cacheKey, async () => {
      try {
        ...
        console.log(`Attempting to scrape airing anime with Puppeteer for page ${page}`);
   ...
   */
  private static async scrapeAiringWithPuppeteer(pageNum: number = 1): Promise<any[]> {
    let browser: Browser | null = null;
    let page: Page | null = null;

    try {
      console.log(`Launching Puppeteer browser for airing anime`);
      ...
      console.log(`Navigating to airing anime URL: ${airingUrl}`);
      ...
      console.log(`Airing anime page content retrieved, length:`, content.length);

      // Parse with Cheerio
      const $ = load(content);

      // Extract anime items
      let animes = scrapeAnimeItems($);

      // Remove duplicates based on id
      const uniqueAnimes = Array.from(new Map(animes.map(anime => [anime.id, anime])).values());

      console.log(`Found ${uniqueAnimes.length} unique airing anime (from ${animes.length} total)`);
