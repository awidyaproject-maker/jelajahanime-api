...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await AiringScraper.getAiringAnime(page, Math.min(limit, 50)) as PaginationData<Anime>;
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Terjadi kesalahan saat mengambil anime yang sedang tayang',
      timestamp: new Date().toISOString(),
    }, { status: 500 });
  }
}